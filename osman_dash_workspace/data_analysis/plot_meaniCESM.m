...
for i = 1:size(tas,4)
    for j = 1:size(tas,1)
        for k = 1:size(tas,2)
            tas_intTS(j,k,i) = nanmean(squeeze(tas(j,k,:,i))) - 273.15;
        end
    end
end
...
for i = 1:size(tos,4)
    for j = 1:size(tos,1)
        for k = 1:size(tos,2)
            tos_intTS(j,k,i) = nanmean(squeeze(tos(j,k,:,i))) - 273.15;
            sos_intTS(j,k,i) = nanmean(squeeze(sos(j,k,:,i)));
        end
    end
end
...
warning('off','all')
    ...
    CT2 = cbrewer('div','RdBu' ,101); CT2 = flipud(CT2);
warning('on','all')
cd(home_fold)
...
ax1 = axes('Position',[0.20 0.675 0.325 0.325]); 
    ...
    colormap(ax1,CT2); 
        c = colorbar('eastoutside'); c.Label.String = [{'Temperature (^oC)'}]; set(c,'Fontsize',11);
        caxis([-globMax globMax]);
...
warning('off','all')
    CT1 = cbrewer('seq','Reds' ,101); 
    % CT1 = cbrewer('div','RdBu' ,101); CT1 = flipud(CT1);
warning('on','all')
cd(home_fold)
...
ax1 = axes('Position',[0.20 0.675 0.325 0.325]); 
    ...
    pcolorm(latPOP,lonPOP,currMap); shading flat;
    colormap(ax1,CT3); 
        c = colorbar('eastoutside'); c.Label.String = [{'PSU (g/kg)'}]; set(c,'Fontsize',11);
